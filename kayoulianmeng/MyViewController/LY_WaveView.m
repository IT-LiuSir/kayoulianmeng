...
//        NSLog(@"当前高度%f",currentWavePointY);
//    }
...
-(void)setCurrentSecondWaveLayerPath {
    ...
    for (float x = 0.0f; x <=  waterWaveWidth ; x++) {
        // 余弦波浪公式
        y = self.waveAmplitude * cos(self.waveCycle * x + offsetX/2) + currentWavePointY;
        CGPathAddLineToPoint(path, nil, x, y);
    }
